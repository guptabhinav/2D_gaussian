...
REAL*8, ALLOCATABLE :: h(:),  sx(:), sy(:), xtau(:), ytau(:)
INTEGER, ALLOCATABLE :: bins(:)

REAL*8 :: ds, alpha, disp2, x, y, vb
...
DO i=1,mtd_step
  READ(2,*)   dummy1, h(i), ds, xtau(i), ytau(i)
  write(10,*) dummy1, h(i), ds, xtau(i), ytau(i)
END DO
   ...
   x=sx(1)+sx(3)*float(lx-1)                      
       DO ly=1,bins(2) 
         ...
         vb=0.0                                                                         !    IF x(l,2).le.x(l,2)+ABS(bin_size) THEN
           DO i=1,mtd_step
           disp2=((x-xtau(i))**2+(y-ytau(i))**2)*0.5   ! exponential term for bias
           vb=vb+h(i)*dexp(-disp2/ds**2)
           END DO
